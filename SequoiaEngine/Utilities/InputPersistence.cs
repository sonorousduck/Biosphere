...
namespace SequoiaEngine
{
    public static class InputPersistence
    {
        ...
        private static async void FinalizeAsyncKeyboardSave(Input input)
        {
            await Task.Run(() =>
            {
                using (IsolatedStorageFile storage = IsolatedStorageFile.GetUserStoreForApplication())
                {
                    try
                    {
                        using (IsolatedStorageFileStream fs = storage.OpenFile("KeyboardControls.json", FileMode.Create))
                        {
                            if (fs != null)
                            {
        ...
        private static async void FinalizeAsyncMouseSave(MouseInput input)
        {
            await Task.Run(() =>
            {
                using (IsolatedStorageFile storage = IsolatedStorageFile.GetUserStoreForApplication())
                {
                    try
                    {
                        using (IsolatedStorageFileStream fs = storage.OpenFile("MouseControls.json", FileMode.Create))
                        {
                            if (fs != null)
                            {
        ...
        private static async void FinalizeAsyncControllerSave(ControllerInput input)
        {
            await Task.Run(() =>
            {
                using (IsolatedStorageFile storage = IsolatedStorageFile.GetUserStoreForApplication())
                {
                    try
                    {
                        using (IsolatedStorageFileStream fs = storage.OpenFile("ControllerControls.json", FileMode.Create))
                        {
                            if (fs != null)
                            {
        ...
        private static async void FinalizeAsyncKeyboardLoad()
        {
            await Task.Run(() =>
            {
                using (IsolatedStorageFile storage = IsolatedStorageFile.GetUserStoreForApplication())
                {
                    try
                    {
                        using (IsolatedStorageFileStream fs = storage.OpenFile("KeyboardControls.json", FileMode.Open))
                        {
                            if (fs != null)
                            {
        ...
        private static async void FinalizeAsyncMouseLoad()
        {
            await Task.Run(() =>
            {
                using (IsolatedStorageFile storage = IsolatedStorageFile.GetUserStoreForApplication())
                {
                    try
                    {
                        using (IsolatedStorageFileStream fs = storage.OpenFile("MouseControls.json", FileMode.Open))
                        {
                            if (fs != null)
                            {
        ...
        private static async void FinalizeAsyncControllerLoad()
        {
            await Task.Run(() =>
            {
                using (IsolatedStorageFile storage = IsolatedStorageFile.GetUserStoreForApplication())
                {
                    try
                    {
                        using (IsolatedStorageFileStream fs = storage.OpenFile("ControllerControls.json", FileMode.Open))
                        {
                            if (fs != null)
                            {
